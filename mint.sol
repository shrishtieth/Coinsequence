...
contract Coinsequence is ERC721, Ownable, ReentrancyGuard {
	using Counters
	for Counters.Counter;
	Counters.Counter tokenId;

	uint256 public tokensPerNft = 5000;
	...
	event minted(uint256 id, address student);
	event burnt(uint256 id, address student);
	event UriUpdated(uint256 id, string uri);
	event SetSigner(address _signer);
	event SetTokensPerNft(uint256 tokens);
	...
	{
		require(!usedNonce[nonce], "Nonce used");
		require(student == ownerOf(id), "Invalid student");
		require(
			matchSigner(
				hashBurn(student, id, nonce),
				signature
			),
			"Not allowed to burn"
		);
		usedNonce[nonce] = true;
		_burn(id);
		emit burnt(id, student);

	}

	// function mintWs(address student, string memory uri)
	// external
	...
	// function burnWs(address student, uint256 id)
	// external
	...
	function hashMint(
		address user, string memory uri,
		string memory nonce
	) public pure returns(bytes32) {
		bytes32 hash = keccak256(abi.encodePacked(user, uri, nonce));
		return hash;
	}

	function hashBurn(
		address user, uint256 id,
		string memory nonce
	) public pure returns(bytes32) {
		bytes32 hash = keccak256(abi.encodePacked(user, id, nonce));
		return hash;
	}

	function hashUpdateUri(
		uint256 id, string memory uri,
		string memory nonce
	) public pure returns(bytes32) {
		bytes32 hash = keccak256(abi.encodePacked(id, uri, nonce));
		return hash;
	}
